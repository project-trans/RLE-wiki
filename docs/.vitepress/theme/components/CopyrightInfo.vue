...
function searchClosestInTrie(that: Trie<Record<string, any>>, path: string[], node: Node<Record<string, any>> = that.root): Record<string, any> | null {
    if (path.length == 0) {
        return node.value
    } else {
        if (path[0] in node.childs) {
            let value = searchClosestInTrie(that, path.slice(1), node.childs[path[0]])
            if (value == null) {
                value = node.value
            }
            return value
...
const pathes = useData().page.value.relativePath.split('/').filter((item: string) => item !== '');
